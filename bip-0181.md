...
**treerows(numleaves):** Returns the minimum number of bits required to represent `numleaves - 1`. This is the height of the smallest perfect tree big enough to hold all numleaves leaves. Returns `0` if `numleaves` is `0`.

This is a property of how positions are labelled, not the height of the largest tree actually in the forest. The two are equal only when `numleaves` is a power of two. When `numleaves` is not a power of two, the enclosing tree is one row taller than the largest tree in the forest and that top row is empty.

The reason for taking the bits of `numleaves - 1` and not `numleaves` is that when `numleaves` is a power of two, using `numleaves` would give a value one too large.

The accumulator with `numleaves = 4` is illustrated below. It is a single 4-leaf tree of height `2`, so `treerows(4)` returns `2`. Because 4 is a power of two, this is also the height of the largest tree in the forest. Using `numleaves` would give `3`, which is incorrect. Using `numleaves - 1` gives the correct `2`.
...
The accumulator with `numleaves = 7` is illustrated below. The forest holds three trees: a 4-leaf tree (height `2`), a 2-leaf tree (height `1`) and a 1-leaf tree (height `0`). The largest tree in the forest has height `2`, but the 7 leaves are labelled inside the smallest enclosing perfect tree, which holds `8` leaves and has height `3`, so `treerows(7)` returns `3`. Positions `07`, `11`, `13` and `14` hold no node and they exist only so the position arithmetic has a full, uniform tree to work in.
