...
This set is typically stored in a database that must be accessed frequently and unlike block data, must be referenced in order to validate future blocks and transactions. As a result, the cost of running a full node is directly tied to the size
...
Utreexo is a dynamic cryptographic accumulator that enables the UTXO set to be represented in just a few kilobytes, by requiring peers that support Utreexo to provide inclusion proofs for the UTXOs being spent. This allows for the construction of nodes that perform the same validation as existing full nodes, without the need to store the entire UTXO set.
